...
> **Stable release:** `1.1.1` is the current stable release. Windows users can install and configure it through the guided installer without editing YAML. The [user manual](./docs/user/en/README.md), this README, `vesicle doctor`, and the examples under [`docs/examples/`](./docs/examples/) remain the supported references.
...
`1.1.1` publishes to npm's `latest` dist-tag, so the unversioned install command above installs the current stable release. Earlier prerelease builds remain available through the `next` dist-tag and explicit versions.
...
The current provider protocols are OpenAI-compatible Chat Completions, Anthropic Messages, Gemini `generateContent`, and the opt-in Responses adapter, which graduated from experimental to released with 1.0.0-alpha.10. Responses configuration is explicit: `openai-public` is the official OpenAI application-layer profile, `codex-beta-2026-02-06` is a fingerprint-level Codex V2 simulation profile, `codex-http-relay` is the HTTP-only maximum-compatibility profile for Codex-serving gateways, and the dated MiMo and DeepSeek profiles are third-party subsets with narrower request and event families. Vesicle never infers a tier from a URL or model name. Model entries may declare generation defaults, capability metadata such as vision or remote-compaction support, and context limits. See the annotated example registry and the [provider configuration reference](./docs/user/en/reference/configuration.md#openai-responses-profiles) for the canonical shape and limitations.
...
- A responsive OpenTUI interface — Chat page plus a Workspace file-workbench page (tree, viewer, editor, in-page validation) — with durable sessions, command completion, provider/model switching, engine handoff, user questions, and confirmation gates; page switching stays available under any prompt, and mid-turn decision prompts collapse to a one-line pending strip on the Workspace page.
...
- Four coarse tool approval modes plus an opt-in non-interactive `shell_exec` process runtime with host-owned PowerShell, CMD, Git Bash, Bash, Zsh, Fish, Nushell, and POSIX shell profiles, exact interpreter-bound plan approval, filtered environment, bounded UTF-8 live output, timeout, process-tree cleanup, foreground/background execution, durable `shell-N` task state, completion notifications that automatically wake an idle session, and explicit output/stop controls.
- An Agent Skills runtime with four-scope discovery, an immutable versioned Skill Store, repository install/update/rollback, and model-visible activation through `/skill` — picker, name completion, and `refresh` to re-freeze a session's catalog. An activated Skill mounts as a read-only `skills/` root that the ordinary read tools, including `grep_files`, can search.
