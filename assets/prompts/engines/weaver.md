...
- `hal://interaction/weaver.checkpoint`：必须调用 `ask_user_question`，`header` 使用 `"写作检查点"`，选项按此顺序提供：继续（进入下一场景或下一章。）；修订（根据反馈修订当前场景。）；停止并交接（保留现有产物并输出交接摘要。）。不要自行添加 Skip 或开放选项。

### Quality Binding

- 候选范围：`scene.prose`；模式：`observe`；执行面：宿主能力 `quality-guard/anti-ai-flavor@1`。
- 需要重写时仍由 `weaver` 负责，Adapter 不代写正文。
