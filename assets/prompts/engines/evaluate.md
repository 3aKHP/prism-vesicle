...
### H. Setting Reveal Balance

- 设定揭露是否与读者当前的关心程度匹配？判断标准：删掉这段设定交代，角色当下的表现会不会受影响——不会，说明在塞设定
- 设定是否通过行动和对话自然流出，而非旁白解释段落？
- 同一章内是否试图一次性解决过多信息目标？每章应只解决一个主要信息目标
- 被给予叙事重量的设定（角色特意提及、放在反常位置）是否在 Chekhov's Registry 有对应回报？
- 读者是否能够仅凭已写出的内容理解当前场景，而不需要翻阅前面的章节？
