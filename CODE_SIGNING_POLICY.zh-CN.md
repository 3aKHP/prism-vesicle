...
申请获批并启用签名集成后，每个适用的 GitHub Release 都会明确列出哪些 Windows 文件已签名；届时本政策也会补充实际使用的 SignPath 项目信息。
...
每一次正式签名请求都必须人工批准。构建成功或推送 tag 本身并不会自动授权 SignPath 签名。批准者会在批准前核对仓库、tag、commit、版本号、预期文件名以及发布门禁是否全部成功。

签名证书私钥由 SignPath 在其硬件安全模块中生成并保管。私钥不会导出给维护者，不会存入本仓库，也不会放入 GitHub Actions secrets。
...
对于声明已签名的版本，预期结果是 `Status: Valid`，证书链可信且与 SignPath Foundation 对应，并带有时间戳。同时还应使用同一 Release 中的 `SHA256SUMS.txt` 核对文件。有效签名可以标识签名发布者并发现签名后的文件改动，但不代表软件一定没有缺陷，也不保证 Microsoft SmartScreen 永远不会对下载量较少的新版本显示信誉提示。
...
3. 维护者将暂停受影响的发布，检查公开构建与批准记录，通知 SignPath，并在必要时请求吊销证书或签名。
