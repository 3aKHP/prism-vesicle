...
import { collect, completed, nativeItemCount, resolveResponsesAcceptance, runResponsesFunctionLoop } from "./responses-support";

const precondition = await resolveResponsesAcceptance({
  providerEnv: "BUN_E2E_MIMO_RESPONSES_PROVIDER",
  modelEnv: "BUN_E2E_MIMO_RESPONSES_MODEL",
  profile: "mimo-subset-2026-07-30",
});
if (!precondition.config) console.log(`[acceptance:mimo-responses] unavailable: ${precondition.reason}`);
const liveTest: typeof test = precondition.config ? test : test.skip;

liveTest("real MiMo subset maps provider reasoning events and Items", async () => {
  ...
  summarize("mimo-responses-reasoning", {
...
liveTest("real MiMo subset completes the declared function-call loop", async () => {
  const config = precondition.config!;
  const adapter = new OpenAIResponsesAdapter({ ...config, responsesTransport: "http" });
  ...
  summarize("mimo-responses-tools", {
    provider: config.providerId,
    model: config.model,
