...
async function runOneTurn(rootDir: string): Promise<{ snapshot: SideQuestionContextSnapshot; requestBody: string; sessionId: string }> {
...
describe("persistent instructions reach the provider system authority", () => {
  test("user then project instruction blocks follow the engine prompt, in order", async () => {
    ...
    const configDir = dirname(process.env.VESICLE_PROVIDERS_FILE!);
    await writeFile(join(configDir, "VESICLE.md"), "USER-BODY-MARKER", "utf8");
  ...
  test("an Engine-specific user target replaces the user general target", async () => {
    await configureTestProviderEnv();
    const configDir = dirname(process.env.VESICLE_PROVIDERS_FILE!);
    await writeFile(join(configDir, "VESICLE.md"), "USER-GENERAL", "utf8");
    await writeFile(join(configDir, "VESICLE.etl.md"), "USER-ETL-OVERRIDE", "utf8");
    const rootDir = await createPromptRoot();
  ...
  test("a new session records the instruction resolution in the system record metadata", async () => {
    await configureTestProviderEnv();
    const configDir = dirname(process.env.VESICLE_PROVIDERS_FILE!);
    await writeFile(join(configDir, "VESICLE.md"), "USER-BODY", "utf8");
    const rootDir = await createPromptRoot();
