...
describe("prompt loading", () => {
  test("loads the capability-aligned host brief and ETL prompt", async () => {
    ...
    expect(profile.systemPrompt).toEqual([
      "assets/prompts/host/authoring.md",
      "assets/prompts/engines/etl.md",
    ]);
    expect(bundle.sections).toHaveLength(2);
    expect(bundle.sections[0].text).toContain("当前工作面");
    expect(bundle.sections[1].text).toContain("Prism ETL Engine");
    expect(bundle.sections[1].text).toContain("hal://interaction/etl.phase");
  ...
  test("composes one prompt from bundled, user, and project asset layers", async () => {
    ...
    try {
      ...
      await mkdir(join(config, "assets", "prompts", "host"), { recursive: true });
      await writeFile(join(project, "assets", "prompts", "engines", "etl.md"), "Project ETL", "utf8");
      await writeFile(join(config, "assets", "prompts", "host", "authoring.md"), "User authoring brief", "utf8");
      ...
      expect(composeSystemPrompt(bundle)).toBe("User authoring brief\n\nProject ETL");
    } finally {
