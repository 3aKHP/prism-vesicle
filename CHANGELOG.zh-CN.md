...
- **展开详情中的鼠标滚轮不再回弹或抖动。** 原生滚轮移动现在会在帧同步前更新共享阅读位置,使位置指示、方向键、终端缩放和重新打开的详情都与滚动后的文本保持一致。
- **展开详情正确渲染 Markdown,标题只显示一次(对 #320 的后续修复)。** 停止门摘要、问题正文及选项描述复用共享 Markdown 渲染器,保留标题、强调、列表、表格和代码块。标题随正文换行和滚动,长标题也能完整阅读。滚动与缩放恢复按渲染后的文本位置定位;权限命令和 JSON 仍按字面文本显示。
- **发布 close-issues 桥接流程跳过提交标题中的普通 issue 引用。** 对末尾的 `(#N)`,现在先确认资源类型再读取 PR 正文,普通 issue 引用不再因 `GET /pulls/N` 返回 404 而中断全部关闭操作。API 故障仍会使工作流失败,不会静默丢弃候选项。
- **Shell 子进程继承 `APPDATA` 和 `XDG_CONFIG_HOME`(#316)。** 通过 `shell_exec` 自调用 `vesicle` 时,现在可以找到 Windows 标准配置目录和自定义 XDG 配置目录。环境策略版本升至 2:版本 1 的待审批请求仍可恢复和拒绝,但点击允许会拒绝执行;请拒绝旧请求并发起新调用。Vesicle 专用配置覆盖变量仍被过滤,Skill 宿主注入机制不变。
...
- **`/skill` 名称补全不再建议本会话无法激活的 Skill(issue #312)。** `/skill <名称>` 的 Tab 补全弹层此前仍按新鲜磁盘扫描构建建议,而激活按会话冻结目录解析 —— 与 #309 修复前的选择器是同一个漂移缺口:resume 更早的会话或会话中途改动 Skill 后,弹层会建议提交即报 `Unknown skill` 的名称。补全现在与选择器共用同一个只读会话目录解析(冻结优先加快照预览路径上的单一宿主端口,绝不创建会话),建议列表、选择器与激活终于同源。无漂移会话与全新会话的建议与之前一致。同时把 `STATUS.md` 的 `/skill` 枚举补齐到包含 `/skill refresh` 与选择器的会话目录列表来源。
- **`/skill` 选择器不再列出本会话无法激活的 Skill(issue #309)。** 选择器此前按新鲜磁盘扫描构建列表,而激活按会话冻结目录解析,于是只要安装内容与冻结快照漂移 —— 会话开始后新装的 Skill(报告场景是 resume 更早录制的会话),或跨重启在会话中途被改动的正文 —— 选择器就会列出按 Enter 后报 `Unknown skill` 的条目,并给已变化的 Skill 展示它随后无法激活的新描述。选择器现在经只读 peek 走激活的同一条解析路径:进程内冻结原样返回,否则按持久化快照 name+hash 重解析,漂移或新装内容被排除出列表。打开选择器绝不写冻结 map、也绝不创建会话;首回合之前仍列出新鲜目录,因此无漂移会话与全新会话的列表与之前一致。`/skill refresh` 仍是按当前安装内容显式重冻结的入口。
- **Harness identity 未变时发生的 Skill 内容漂移不再让长期会话永久失去该 Skill:`/skill refresh` 提供显式重冻结入口(issue #308)。** 冻结目录契约此前没有任何非迁移的写入路径,于是一旦某个 Skill 的正文在 Harness identity 不变的前提下发生变化 —— 直接编辑用户目录,或一次只更新 `host-assets/skills/` 而未触碰 harness manifest 的宿主资产更新 —— 该 Skill 就在恢复时被静默移出会话目录,`/skill <名称>` 从此永远报 `Unknown skill`;反向同样成立:会话开始后新安装的 Skill 永远进不了目录。`/skill refresh` 按当前安装内容重冻结会话目录,复用迁移重冻结机制:输出变更(带重激活指引)、移除与新增报告;新快照作为 append-only 的 `skill-catalog` 记录持久化,按会话级物理序 latest-wins 解析(对任意分支头一致);旧 hash 激活经既有名称加内容哈希规则自然退役,`/skill <名称>` 重激活不被去重压制;无漂移时为幂等 no-op。Harness identity 未变的漂移会话在恢复时会看到一条指向该命令的提示 —— 恢复本身绝不写目录,重冻结保持显式,与迁移的两阶段确认同级。
