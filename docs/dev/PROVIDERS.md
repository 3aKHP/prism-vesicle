...
- A proxy `407` is terminal for its attempt class and surfaces a stable `proxy_authentication_required` code; known-bad credentials are not retried. The WebSocket authentication preflight uses the same bounded request timeout as the socket lifecycle, so a proxy that accepts CONNECT but never answers cannot hold the turn indefinitely. A proxy connection failure is a transport attempt and follows the existing retry policy with a sanitized, non-revealing `proxy_connect_failed` code; the proxy URL, host, port, and credentials never enter error messages, causes, diagnostics, or session/quality artifacts.
...
- Gemini adapters map the system prompt to `systemInstruction`, conversation to `contents`, and tool results to `functionResponse` parts. A `user` Content holds either `functionResponse` parts only or ordinary multimodal (text/`inlineData`) parts only; consecutive function responses from one model turn remain in one response batch, tool-result images serialize as separate following ordinary `user` Contents, and later ordinary user messages are never merged into a function-response Content. Mixing the part kinds in one Content, or splitting a parallel response batch, is rejected by the endpoint.
...
- Gemini `thought` and `thoughtSignature` metadata remain attached to the original provider-native parts and are replayed as those parts on the next request instead of being reconstructed from assistant prose. The adapter never emits a signature-less part with no initialized data field: Gemini streaming can end a response with a bare empty text part (the documented carrier shape for a trailing signature), and relays that re-serialize JSON can degrade such parts further — stripping the empty string and leaving `{}` or `{thought:true}` — so signature-less parts whose only keys are `text`/`thought` with empty or absent text are dropped at capture and at replay, while a part that carries a signature is replayed verbatim. A message that serializes to zero parts is omitted rather than sent as an empty-text placeholder.
...
- The Responses adapter applies request omission, semantic-event admission, terminal reconstruction, and native Item/compact-state ownership by exact `responsesProfile`. Changing profile at the same provider/model/endpoint selects portable history rather than replaying incompatible native state. `openai-public` owns the public OpenAI field/event families and requires terminal ordered output. The HTTP-only `codex-http-relay` maximum-compatibility profile may reconstruct an empty or omitted terminal output from contiguous `response.output_item.done` Items, but only after a structurally valid `response.completed`; a non-empty terminal representation must match the completed Item stream exactly, and failed/incomplete/EOF attempts commit nothing. The dated MiMo and DeepSeek subsets omit their unsupported stateful fields, replay full context over HTTP, and explicitly admit only their documented `response.reasoning_text.*` family. Unknown semantic events remain fail-closed.
...
- String model entries cover the common case. Object entries may declare `id`, `generation`, `capabilities`, `limits`, and `webSearchDefault`.
...
- Provider-native built-in web search requires all three authorities: `capabilities.builtinWebSearch`, an admitting protocol/profile, and an Engine whose declared model-visible surface includes `web_search`. The model entry's `webSearchDefault` supplies the session default and `/websearch` supplies a per-session override; the command rejects unsupported protocol/profile/Engine combinations instead of reporting a toggle that the wire would ignore. It is model capability, not a host tool: permission modes do not govern it, and it reaches adapters only as the normalized `VesicleRequest.webSearch` declaration flag. Within `openai-responses`, the flag is consumed only by the search-admitting profiles (`openai-public`, `deepseek-subset-2026-08-19`), which declare the bare `web_search` tool, admit `web_search_call` Items and events, and persist the normalized report; see [`OPENAI_RESPONSES_CONFORMANCE.md`](./OPENAI_RESPONSES_CONFORMANCE.md). Gemini `generateContent` consumes the flag by appending a standalone `{googleSearch:{}}` tools entry (including alongside function declarations), then normalizes candidate `groundingMetadata` queries and optional URI/title citations; Gemini does not receive grounding metadata in replay because its protocol defines no replay item. The normalized audit report remains durable through main and child assistant records, Quality decisions, and portable compact checkpoints.
...
- Every `openai-responses` provider declares `responsesProfile`; transport defaults to HTTP and may be set to WebSocket only for a profile that supports it. The dated MiMo subset is HTTP-only, may select Bearer or `x-api-key`, and cannot enable `remoteCompact`. `deepseek-subset-2026-07-31` is Bearer-authenticated, HTTP-only, stateless, and limited to the documented `deepseek-v4-flash` and `deepseek-v4-pro` pair; other DeepSeek models remain rejected. `deepseek-subset-2026-08-19` copies those constraints and additionally admits provider-side built-in web search; the guided Setup wizard writes it as the DeepSeek Responses preset default, while existing configurations keep their declared profile until explicitly changed. Other Responses profiles use Bearer authentication; other protocols retain their existing auth rules.
