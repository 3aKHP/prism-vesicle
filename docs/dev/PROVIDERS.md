...
- Provider-native built-in web search is capability-gated by `capabilities.builtinWebSearch` with a session default from the model entry's `webSearchDefault` and a per-session `/websearch` override. It is model capability, not a host tool: permission modes do not govern it, and it reaches adapters only as the normalized `VesicleRequest.webSearch` declaration flag. Adapters without built-in search support leave the flag inert. No adapter consumes the flag yet; the declaration is groundwork for [#225](https://github.com/3aKHP/prism-vesicle/issues/225).
