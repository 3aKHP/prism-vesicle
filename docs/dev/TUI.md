...
- Decision prompts with folded body text (gate summary, permission detail, question text) carry a two-zone keymap (`#268` item 4): `Tab`/`Shift+Tab` toggle between the options zone and a body zone where `↑/↓` scroll, `Home`/`End` jump, and `Tab`/`Enter`/`Esc` return — Enter never submits from the body zone. Plain typing or paste on a focused confirm option auto-arms its note (the explicit "Tab note" toggle is gone); the fold marker becomes an accent affordance row in the options zone (`▸ N lines folded · Tab to read`) and a bright position row in the body zone (`▾ lines 5-8 of 30`); the rail column beside the body is dim in the options zone and brand-colored in the body zone, and a new prompt resets zone and offset. The scroll extent is reported by the rendering component (`registerBodyExtent`), so the offset clamps to the real window.
