...
- Picker panels and self-opened confirms (YOLO, migration review, rewrite confirm) own the bottom area while active on both pages. The four mid-turn decision prompts (permission, gate, question, quality) own the bottom on the Chat page; on the Workspace page they collapse to a one-line pending strip above the composer (`#268` item 3) — the workbench keeps its rows and its three-region keyboard, the full panel answers on Chat, and the composer refuses Enter (no send, no queue; draft kept) until the decision resolves. Side panes may be hidden so the active controls remain legible.
