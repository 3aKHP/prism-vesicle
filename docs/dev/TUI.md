...
- Decision prompts with folded body text (gate summary, permission detail, question text) carry a two-zone keymap (`#268` item 4): `Tab`/`Shift+Tab` toggle between the options zone and a body zone where `↑/↓` scroll, `Home`/`End` jump, and `Tab`/`Enter`/`Esc` return — Enter never submits from the body zone. Plain typing or paste on a focused confirm option auto-arms its note (the explicit "Tab note" toggle is gone); the fold marker is a scroll position indicator (`… lines 5-8 of 30`), and a new prompt resets zone and offset. The scroll extent is reported by the rendering component (`registerBodyExtent`), so the offset clamps to the real window.
