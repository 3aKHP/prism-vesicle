...
- OpenTUI 0.4.3 does not expose Markdown selection colors, so the pinned dependency carries a narrow Bun patch that adds `selectionBg`/`selectionFg` to `MarkdownOptions` and propagates them to prose, list markers, fenced code, and tables. Source and standalone builds replay the tracked patch through `patchedDependencies`; the precompiled npm entry bundles the patched core JavaScript while leaving the parser worker and platform-native packages external, so installed consumers do not depend on package-manager patch support or load a second unpatched core. The `debug markdown-runtime` diagnostic performs real native Markdown drag selection through the linked runtime, and package/binary/installer smoke runs it at each distribution boundary. Remove the patch and its `patchedDependencies` metadata only once the pinned upstream release provides equivalent public options and propagation and those native diagnostics still pass; [Issue #172](https://github.com/3aKHP/prism-vesicle/issues/172) tracks the upstream link and deletion condition after the user-visible defect in Issue #158 closes.
- Markdown backslash escapes (`\~`, `\*`, …) render as the escaped character through an interim host-side transform in `src/tui/markdown-escape-conceal.ts`: the upstream OpenTUI highlight query styles `(backslash_escape)` with no conceal and the atomic grammar token cannot be split by any query rule (upstream issue [anomalyco/opentui#1369](https://github.com/anomalyco/opentui/issues/1369), PR [#1370](https://github.com/anomalyco/opentui/pull/1370)). The transform wraps `TreeSitterClient.prototype.highlightOnce` and conceals the backslash byte; it is installed from `tree-sitter-runtime`, `MarkdownContent`, and the markdown-runtime diagnostic (compiled binaries never import the runtime module), and ships inside Vesicle's own JavaScript so every distribution channel — source, npm bundle, standalone binaries — gets it without package-manager patch support. It covers the tree-sitter markdown surfaces (Chat messages, Workspace markdown previews, side questions); plain-text mode and artifact-card previews decode escapes in the preprocessing layer instead. The `escape` field of `vesicle debug markdown-runtime` asserts the concealed tuples at every distribution smoke, and `tests/component/tui/markdown-escape.test.tsx` asserts the rendered frame. It is removed together with the 0.4.3 dependency patch when the self-maintained OpenTUI fork baseline (which carries the proper worker fix) is adopted.
