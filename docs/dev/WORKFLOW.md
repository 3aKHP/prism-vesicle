...
The real-provider gate acceptance test is intentionally opt-in because it is model-output dependent: run `BUN_E2E_REAL_PROVIDER=1 bun test ./tests/acceptance/provider/e2e-gate.acceptance.ts` from the trusted dogfood environment and record the result before creating a public tag. It validates a real provider without making ordinary CI depend on provider credentials, model determinism, or unapproved API spend.
