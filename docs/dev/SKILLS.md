...
- `/skill` TUI command: bare `/skill` opens a compact picker listing exactly the session's activatable catalog — resolved read-only through the same freeze-then-snapshot path activation uses, so a Skill that drifted since the freeze or was installed after it is not listed rather than failing on Enter with `Unknown skill` (#309); `/skill <name> [task]` activates and invokes; `/skill <name> --context-only` activates without a provider request; `/skill refresh` (#308) re-freezes the session catalog at the current installation content. The reserved subcommand executes without a confirmation panel (it queues at the Agent Loop boundary like activation), reports changed/removed/added, appends nothing when nothing drifted, and shadows a Skill literally named "refresh".
- Session semantics: the catalog is frozen per session (resume re-resolves by name+hash, never silently substituting changed content). The frozen snapshot is session-level Host state projected in physical record order, so branch truncation (regenerate, rewind, candidate switching) cannot resurrect an older catalog. An activation is live only while the frozen catalog serves the same content hash it was recorded with; a confirmed Harness migration re-freezes the catalog at the current installation's content, which retires activations recorded at a stale hash until `/skill` re-activates them — the old activation record stays in history as an audit trail. `/skill refresh` (#308) is the explicit non-migration re-freeze: it appends a `skill-catalog` record under the same session-level latest-wins projection and retires stale-hash activations the same way, while resuming a drifted session whose Harness identity did not change surfaces an advisory hint pointing at the command — the resume itself never writes the catalog. The re-freeze resolves under the session's recorded provider-selection config — the budget source the next bootstrap's re-resolution uses — so it keeps everything the refresh froze. Activation records persist as user records with `kind: "skill-activation"`. Compaction reattaches active Skill bodies within a 16 KiB budget or reports loss. Rewind removes activation state only when the record is beyond the new head. Engine switching recomputes eligibility and prunes the activation registry.
...
- Project `.agents/skills/` discovery: Skills under `<project-root>/.agents/skills/<name>/SKILL.md` enter the catalog with visible `project` scope attribution. No separate project-trust gate; installation into the project directory is the authorization. Precedence: `project` > `user` > `installed` > `harness` > `host`.
