...
- `run_skill_script(skill, path, args[])` tool: executes a fixed script from an activated Skill through the Process Runtime as structured argv (no shell interpolation). It is available for every non-Stage session with a non-empty Skill catalog, independent of the opt-in `shell_exec` capability and `shellInterpreter`; the required interpreter is resolved from the script extension at execution time. It retains the shared environment filtering, timeout, output cap, cancellation, and process-group cleanup, while its distinct `skill_exec` permission class asks in MANUAL/INERTIA and auto-allows in MOMENTUM/YOLO.
