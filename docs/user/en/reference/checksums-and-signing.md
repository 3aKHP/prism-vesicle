...
**Current status (`1.0.0-alpha.4`)**: the Windows executable and installer are **intentionally unsigned**. Windows signing is deferred until the project has a stronger basis for a signing provider, with no version deadline. So:
