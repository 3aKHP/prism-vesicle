...
两个路径都必须相对于当前项目，并位于允许的项目根目录下。Vesicle 会在创建会话前验证两张卡，然后在你的第一次行动前保存渲染后的角色上下文和开场场景。Stage 默认没有模型可见工具、确认门、MCP 工具或自动重写。之后即使编辑了任一源卡，恢复的 Stage 会话仍会使用已保存的角色和场景上下文，并可能提示源文件已变化。

Stage 回复使用共享的三段式 packet。面向玩家的隐藏推理与 HUD 完整渲染在当前 alpha 中仍待后续交付，因此不要把原始 packet 可见性视为最终消费体验。
