...
The installer is compiled with the deterministic inputs under `brand/windows/`: the canonical multi-size `prism-vesicle.ico` is also installed beside `vesicle.exe` so Apps & Features, the generated uninstaller, Start Menu, and Explorer integration can consume the full-size icon directly instead of scaling a low-resolution PE frame; `prism-vesicle-wizard.png` supplies the Inno Setup wizard image and is not installed. Build the branded PE and installer on native Windows from a drive-letter workspace; Windows Bun is rejected when the source checkout is reached through a UNC path (including `\\wsl.localhost`) because that layout relocates the embedded Worker entrypoint. A WSL cross-build is deliberately named `prism-vesicle-cross-windows-x64.exe` and is not a release artifact because Bun cannot write Windows PE resources while cross-compiling.
