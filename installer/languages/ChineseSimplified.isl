...
MaintenanceCaption=Prism Vesicle 维护
MaintenanceDescription=选择如何维护现有安装。
MaintenanceSubCaption=重新安装会更新全部程序文件；修复会恢复程序文件与 Windows 集成，但不会重新打开引导式设置；卸载会启动现有卸载程序。
MaintenanceReinstall=重新安装 / 升级 Prism Vesicle
MaintenanceRepair=修复程序文件与 Windows 集成
MaintenanceUninstall=卸载 Prism Vesicle
MaintenanceUninstallFailed=无法启动现有 Prism Vesicle 卸载程序：
