...
- `windows/` — deterministic Windows distribution derivatives. `prism-vesicle.ico` uses the mono silhouette at 16–48 px and the transparent compact mark at 64–256 px; `prism-vesicle-wizard.png` is the 256 px compact mark used by the Inno Setup wizard. Run `bun run build:windows-icon` to regenerate and `bun run check:windows-icon` to verify byte-for-byte determinism.
