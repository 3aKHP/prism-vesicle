...
if ($args.Count -lt 1) {
  [Console]::Error.WriteLine("Usage: update_config.ps1 <path|show|set|add-provider|add-model|add-mcp|remove-mcp|remove-model|remove-provider|unset|env-set-empty|env-set-proxy|env-remove|validate> [args...]")
  exit 2
}
