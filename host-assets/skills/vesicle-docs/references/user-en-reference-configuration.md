...
The independent Responses protocol remains opt-in experimental in the current alpha; the full `openai-public` real-provider gate passed on 2026-08-11 across HTTP/typed SSE, non-stream JSON, standalone compact, and public WebSocket (`3` pass, `0` fail), and is slated to graduate from experimental in the next release. On 2026-07-31, the funded MiMo endpoint and DeepSeek v4 Flash each passed their reasoning and function-loop cases (`2` pass, `0` fail per subset).
