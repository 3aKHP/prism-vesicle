...
- 写操作只允许落在这些根下:`source_materials/`、`workspace/`、`novels/`、`reports/`、`test_runs/`,以及暂存根 `tmp/`。
  ...
  - `tmp/` 是项目相对的暂存根(`<项目>/tmp/`,不是操作系统 `/tmp`),用于草稿和中间工作;它受同样的路径守卫与权限模式约束,其改动可写入但不纳入回合级文件检查点/回退,因此暂存区编辑不可回退。它不会进入制品列表、`/validate`、`/init`、Stage 输入或自动发布。宿主不会自动创建或清空 `tmp/`;需要清理时请显式删除文件。
