<!-- Generated from docs/user/zh-CN/reference/permissions-and-security.md — do not edit. -->
...
- 读操作另可到达解析器承载的 `assets/` 命名空间,以及会话内已激活 Skill 的只读 `skills/` 挂载:`skills/<名称>/...` 在该 Skill 的虚拟根内解析,护栏与 256 KiB 文本上限与 `read_skill_resource` 相同。挂载严格只读——`skills/` 下的写入、移动、复制目标与删除一律拒绝。
- 写操作只允许落在这些根下:`source_materials/`、`workspace/`、`novels/`、`reports/`、`test_runs/`,以及暂存根 `tmp/`。
  - `source_materials/` 存放导入、研究或模型生成的素材;最终产物落在其余四个根。
  - `tmp/` 是项目相对的暂存根(`<项目>/tmp/`,不是操作系统 `/tmp`),用于草稿和中间工作;它受同样的路径守卫与权限模式约束,其改动可写入但不纳入回合级文件检查点/回退,因此暂存区编辑不可回退。跨 `tmp/` 边界的移动在回退时不可完全逆转:从 `tmp/` 移入内容根的文件会被删除且无法恢复;从内容根移入 `tmp/` 的文件会还原到原位,而 `tmp/` 中的副本仍保留。若可能回退,请用 `copy_file` 提升暂存内容。它不会进入制品列表、`/validate`、`/init`、Stage 输入或自动发布。宿主会在新会话启动时创建缺失的可写根(包括 `tmp/`;恢复旧会话不会重建你已删除的根),但从不自动清空 `tmp/`;需要清理时请显式删除文件。
...
- 等待批准期间,转录会保留一行 `Permission pending` 并附命令的单行摘要;批准后,这次执行显示为标准的 `●` 命令卡,与结果行成对出现。
