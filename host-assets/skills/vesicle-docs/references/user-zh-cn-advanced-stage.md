<!-- Generated from docs/user/zh-CN/advanced/stage.md — do not edit. -->

# Stage 消费引擎

[English](../../en/advanced/stage.md) | 简体中文

> **状态(截至 `1.0.0-beta.2`):** 🟢 已实现。成熟度以 [`STATUS.md`](../../../../STATUS.md) 为准。
...
成功后会进入一条**新的 Stage 会话**:对话区先显示情景卡的可见开场,header 的 Engine 变为 Stage,输入框等待你的第一条角色行动。这个启动动作本身不调用供应商;你的第一条行动才开始模型续写。

### 无法启动时

- `Usage: /stage ...`:必须给两个项目相对路径;路径含空格时用双引号包住,例如 `/stage "workspace/角色 卡.md" "workspace/情景 卡.md"`。输入命令后也可用候选补全,它会自动为含空格路径加引号。
- `Paths are project-relative...` 或找不到文件:用 `/workspace <路径>` 逐一确认文件存在于 `workspace/` 等可读根;绝对路径和 `..` 会被拒绝。
- 卡片有兼容 warning:先读完最多三条提示。程序只会提示输入为空、Module A/B 缺少 YAML frontmatter、Module B 缺少可选逻辑注释或 HTML 注释未闭合;它们不会阻断。回到 ETL 按提示修卡后应开一个**新** Stage 会话,进行中的冻结会话不会热更新。
- 启动后供应商报错:Stage 会话和冻结开场已经持久化。修复连接后 `/resume` 该会话,再重发你的第一条行动,不要重复 `/stage` 制造另一条会话。
