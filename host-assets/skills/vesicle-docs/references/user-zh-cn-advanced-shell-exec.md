<!-- Generated from docs/user/zh-CN/advanced/shell-exec.md — do not edit. -->

# 宿主 Shell 与 Process Runtime

[English](../../en/advanced/shell-exec.md) | 简体中文

> **状态(截至 `1.0.0`):** 🟢 已实现。这是一个拥有宿主用户权限的**非沙箱**能力,默认关闭。成熟度以 [`STATUS.md`](../../../../STATUS.md) 为准。
>
...
- **后台**(`runInBackground: true`):立即返回一个任务 id(形如 `shell-N`),不阻塞。进度在 TUI 可见,**完成会自动送回对话,无需轮询**:回合进行中就并入下一个 provider 轮,你在空闲状态时就自动开一个续跑回合接手。相近时间完成的任务会合并成一次投递;投递的 provider 轮失败或被打断会暂停,等你的下一条消息再重试。每次投递都是一次正常的 provider 轮,和其他回合一样消耗 token。输出持久化在项目的 `.vesicle/processes/`。
...
> 重启恢复:Vesicle 重启时,仍在跑的后台任务被**恢复为 interrupted(不重放)**——不会替你把进行中的命令再跑一遍;关机期间已完成的任务,在你恢复会话后仍会送达。
