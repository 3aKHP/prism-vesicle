...
- Tests use a level-first, domain-second structure: `unit/`, `component/`, `integration/`, `contract/`, and `acceptance/`, followed by the subsystem under test. System and release smoke may live in `scripts/smoke/` and CI when that is the real execution boundary; the `scripts/` tree splits into `build/`, `check/`, and `smoke/` roles.
