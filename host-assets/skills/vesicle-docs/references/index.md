<!-- Generated index — do not edit. Run: bun run skills:docs:sync -->

# Vesicle Documentation Index

Version-matched public documentation snapshot for the vesicle-docs Skill.

## Routing guidance

- User-facing questions: prefer the matching-language user manual (`docs/user/zh-CN` or `docs/user/en`).
- Developer/runtime questions: use `docs/dev/` contracts.
- Configuration shapes: `docs/examples/` are authoritative examples, not user state.
- Overview/installation: start from `README.md`.
- If information is missing, report the gap rather than inventing an answer.

## Root

- `README.md` → `references/root-readme.md` — Prism Vesicle [en]

## User Manual

- `docs/user/en/advanced/mcp.md` → `references/user-en-advanced-mcp.md` — MCP tools [en]
- `docs/user/en/advanced/quality-guard.md` → `references/user-en-advanced-quality-guard.md` — Output Quality Guard [en]
- `docs/user/en/advanced/README.md` → `references/user-en-advanced-readme.md` — Advanced and experimental features [en]
- `docs/user/en/advanced/shell-exec.md` → `references/user-en-advanced-shell-exec.md` — Host shell and Process Runtime [en]
- `docs/user/en/advanced/skills.md` → `references/user-en-advanced-skills.md` — Skills [en]
- `docs/user/en/advanced/stage.md` → `references/user-en-advanced-stage.md` — Stage consumer engine [en]
- `docs/user/en/advanced/subagents.md` → `references/user-en-advanced-subagents.md` — SubAgents [en]
- `docs/user/en/README.md` → `references/user-en-readme.md` — Prism Vesicle User Manual [en]
- `docs/user/en/reference/checksums-and-signing.md` → `references/user-en-reference-checksums-and-signing.md` — Checksums and signing [en]
- `docs/user/en/reference/commands.md` → `references/user-en-reference-commands.md` — Command cheatsheet [en]
- `docs/user/en/reference/configuration.md` → `references/user-en-reference-configuration.md` — Configuration files [en]
- `docs/user/en/reference/permissions-and-security.md` → `references/user-en-reference-permissions-and-security.md` — Permissions and security model [en]
- `docs/user/en/reference/README.md` → `references/user-en-reference-readme.md` — Reference [en]
- `docs/user/en/reference/troubleshooting.md` → `references/user-en-reference-troubleshooting.md` — Troubleshooting [en]
- `docs/user/en/reference/update-uninstall-migrate.md` → `references/user-en-reference-update-uninstall-migrate.md` — Update · uninstall · migrate [en]
- `docs/user/en/start/linux-portable.md` → `references/user-en-start-linux-portable.md` — Linux portable [en]
- `docs/user/en/start/npm.md` → `references/user-en-start-npm.md` — npm install [en]
- `docs/user/en/start/windows-installer.md` → `references/user-en-start-windows-installer.md` — Windows installer start [en]
- `docs/user/en/start/windows-portable.md` → `references/user-en-start-windows-portable.md` — Windows portable [en]
- `docs/user/en/tutorials/first-character-card.md` → `references/user-en-tutorials-first-character-card.md` — First character card [en]
- `docs/user/en/tutorials/first-conversation.md` → `references/user-en-tutorials-first-conversation.md` — First conversation: interface and gates [en]
- `docs/user/en/tutorials/first-scenario-card.md` → `references/user-en-tutorials-first-scenario-card.md` — First scenario card [en]
- `docs/user/en/tutorials/permissions-and-shell.md` → `references/user-en-tutorials-permissions-and-shell.md` — Permissions and the host shell [en]
- `docs/user/en/tutorials/persistent-instructions.md` → `references/user-en-tutorials-persistent-instructions.md` — Set up Persistent Instructions for a project [en]
- `docs/user/en/tutorials/README.md` → `references/user-en-tutorials-readme.md` — Tutorials [en]
- `docs/user/en/tutorials/sessions-and-rewind.md` → `references/user-en-tutorials-sessions-and-rewind.md` — Sessions and rewind [en]
- `docs/user/en/tutorials/work-while-running.md` → `references/user-en-tutorials-work-while-running.md` — Keep working during a turn: queue and side questions [en]
- `docs/user/en/tutorials/workspace-page.md` → `references/user-en-tutorials-workspace-page.md` — View and edit artifacts in the Workspace page [en]
- `docs/user/zh-CN/advanced/mcp.md` → `references/user-zh-cn-advanced-mcp.md` — MCP 工具 [zh-CN]
- `docs/user/zh-CN/advanced/quality-guard.md` → `references/user-zh-cn-advanced-quality-guard.md` — Output Quality Guard [zh-CN]
- `docs/user/zh-CN/advanced/README.md` → `references/user-zh-cn-advanced-readme.md` — 高级与实验特性 [zh-CN]
- `docs/user/zh-CN/advanced/shell-exec.md` → `references/user-zh-cn-advanced-shell-exec.md` — 宿主 Shell 与 Process Runtime [zh-CN]
- `docs/user/zh-CN/advanced/skills.md` → `references/user-zh-cn-advanced-skills.md` — Skills [zh-CN]
- `docs/user/zh-CN/advanced/stage.md` → `references/user-zh-cn-advanced-stage.md` — Stage 消费引擎 [zh-CN]
- `docs/user/zh-CN/advanced/subagents.md` → `references/user-zh-cn-advanced-subagents.md` — SubAgents [zh-CN]
- `docs/user/zh-CN/README.md` → `references/user-zh-cn-readme.md` — Prism Vesicle 用户手册 [zh-CN]
- `docs/user/zh-CN/reference/checksums-and-signing.md` → `references/user-zh-cn-reference-checksums-and-signing.md` — 校验和与签名 [zh-CN]
- `docs/user/zh-CN/reference/commands.md` → `references/user-zh-cn-reference-commands.md` — 命令速查 [zh-CN]
- `docs/user/zh-CN/reference/configuration.md` → `references/user-zh-cn-reference-configuration.md` — 配置文件 [zh-CN]
- `docs/user/zh-CN/reference/permissions-and-security.md` → `references/user-zh-cn-reference-permissions-and-security.md` — 权限与安全模型 [zh-CN]
- `docs/user/zh-CN/reference/README.md` → `references/user-zh-cn-reference-readme.md` — 参考 [zh-CN]
- `docs/user/zh-CN/reference/troubleshooting.md` → `references/user-zh-cn-reference-troubleshooting.md` — 故障排查 [zh-CN]
- `docs/user/zh-CN/reference/update-uninstall-migrate.md` → `references/user-zh-cn-reference-update-uninstall-migrate.md` — 更新·卸载·迁移 [zh-CN]
- `docs/user/zh-CN/start/linux-portable.md` → `references/user-zh-cn-start-linux-portable.md` — Linux 便携版 [zh-CN]
- `docs/user/zh-CN/start/npm.md` → `references/user-zh-cn-start-npm.md` — npm 安装 [zh-CN]
- `docs/user/zh-CN/start/windows-installer.md` → `references/user-zh-cn-start-windows-installer.md` — Windows 安装器入门 [zh-CN]
- `docs/user/zh-CN/start/windows-portable.md` → `references/user-zh-cn-start-windows-portable.md` — Windows 便携版 [zh-CN]
- `docs/user/zh-CN/tutorials/first-character-card.md` → `references/user-zh-cn-tutorials-first-character-card.md` — 第一张角色卡 [zh-CN]
- `docs/user/zh-CN/tutorials/first-conversation.md` → `references/user-zh-cn-tutorials-first-conversation.md` — 第一次对话:界面与门 [zh-CN]
- `docs/user/zh-CN/tutorials/first-scenario-card.md` → `references/user-zh-cn-tutorials-first-scenario-card.md` — 第一张情景卡 [zh-CN]
- `docs/user/zh-CN/tutorials/permissions-and-shell.md` → `references/user-zh-cn-tutorials-permissions-and-shell.md` — 权限与宿主 Shell [zh-CN]
- `docs/user/zh-CN/tutorials/persistent-instructions.md` → `references/user-zh-cn-tutorials-persistent-instructions.md` — 为项目建立持久化指令 [zh-CN]
- `docs/user/zh-CN/tutorials/README.md` → `references/user-zh-cn-tutorials-readme.md` — 教程 [zh-CN]
- `docs/user/zh-CN/tutorials/sessions-and-rewind.md` → `references/user-zh-cn-tutorials-sessions-and-rewind.md` — 会话恢复与回退 [zh-CN]
- `docs/user/zh-CN/tutorials/work-while-running.md` → `references/user-zh-cn-tutorials-work-while-running.md` — 运行中继续工作:队列与旁路提问 [zh-CN]
- `docs/user/zh-CN/tutorials/workspace-page.md` → `references/user-zh-cn-tutorials-workspace-page.md` — 在 Workspace 页查看和修改产物 [zh-CN]

## Developer Contracts

- `docs/dev/ARCHITECTURE.md` → `references/dev-architecture.md` — Prism Vesicle Architecture [en]
- `docs/dev/ASSETS.md` → `references/dev-assets.md` — Prism Runtime Assets [en]
- `docs/dev/AUDIT_DRIFT.md` → `references/dev-audit_drift.md` — Audit Database Drift [en]
- `docs/dev/COMMAND_COMPLETION.md` → `references/dev-command_completion.md` — Command Argument Completion [en]
- `docs/dev/COMMAND_QUEUE.md` → `references/dev-command_queue.md` — Command Queue [en]
- `docs/dev/OPENAI_RESPONSES_CONFORMANCE.md` → `references/dev-openai_responses_conformance.md` — OpenAI Responses Conformance Profile [en]
- `docs/dev/PERSISTENT_INSTRUCTIONS.md` → `references/dev-persistent_instructions.md` — Persistent Instructions Runtime Contract [en]
- `docs/dev/PROVIDERS.md` → `references/dev-providers.md` — Provider Runtime Contract [en]
- `docs/dev/QUALITY_BENCHMARK.md` → `references/dev-quality_benchmark.md` — Semantic Judge Benchmarking [en]
- `docs/dev/QUALITY_GUARD.md` → `references/dev-quality_guard.md` — Output Quality Guard [en]
- `docs/dev/README.md` → `references/dev-readme.md` — Public Developer Documentation [en]
- `docs/dev/SESSIONS.md` → `references/dev-sessions.md` — Session And Context Runtime Contract [en]
- `docs/dev/SETUP.md` → `references/dev-setup.md` — Guided Setup And Installer [en]
- `docs/dev/SKILLS.md` → `references/dev-skills.md` — Skills Runtime [en]
- `docs/dev/STAGE.md` → `references/dev-stage.md` — Stage Engine [en]
- `docs/dev/STYLE.md` → `references/dev-style.md` — Prism Vesicle Code Style [en]
- `docs/dev/SUBAGENTS.md` → `references/dev-subagents.md` — SubAgent Runtime Contract [en]
- `docs/dev/TOOLS.md` → `references/dev-tools.md` — Tool And Interaction Runtime Contract [en]
- `docs/dev/TUI.md` → `references/dev-tui.md` — TUI Runtime Contract [en]
- `docs/dev/USER_AGENCY_AND_RISK_DISCLOSURE.md` → `references/dev-user_agency_and_risk_disclosure.md` — User Agency And Risk Disclosure [en]
- `docs/dev/WORKFLOW.md` → `references/dev-workflow.md` — Prism Vesicle Workflow [en]

## Examples

- `docs/examples/mcp.yaml` → `references/examples-mcp-yaml.md` — Copy this optional file to the same user-level directory as providers.yaml: [en]
- `docs/examples/permissions.yaml` → `references/examples-permissions-yaml.md` — Vesicle host tool approval settings. This file contains no secrets. [en]
- `docs/examples/provider.env.example` → `references/examples-provider-env-example.md` — Copy this file to the same user-level directory as providers.yaml. [en]
- `docs/examples/providers.yaml` → `references/examples-providers-yaml.md` — examples-providers-yaml [en]
- `docs/examples/quality.yaml` → `references/examples-quality-yaml.md` — Experimental Semantic Judge settings. This is user-level host configuration, [en]

## Other

- `docs/user/README.md` → `references/user-readme.md` — Prism Vesicle User Manual [en]
