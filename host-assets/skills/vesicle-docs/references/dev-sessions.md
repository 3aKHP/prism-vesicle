...
- Switching candidates applies `pre(target) ∪ pre(departing) ∪ bundle(target)`: the bundle carries the target's real post-state and is authoritative over its capture domain; the departing pre-state contributes deletion entries for files only that candidate created; the target pre-state covers tracked paths outside the bundle's capture domain (for example, records the target branch appended after the bundle was captured). A target without a bundle (produced before coexistence and never departed) degrades to a conversation-only switch.
