...
- Engine, provider/model, reasoning, and permission preferences — plus the effective Harness identity after a confirmed migration and the frozen Skill catalog snapshot — are session-level Host state. Resume projects their latest values from physical JSONL order across the whole session, independently of the selected conversational branch, so regenerating, rewinding, or candidate-switching to a branch recorded before a `session-migration` record still runs under the post-migration baseline and catalog.
