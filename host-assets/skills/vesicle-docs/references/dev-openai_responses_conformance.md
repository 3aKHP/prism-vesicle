...
- MiMo is a capability-declared Responses-compatible subset, not an OpenAI/Codex conformance profile. Its frozen 2026-07-30 profile omits unsupported continuation/compaction fields and explicitly owns `response.reasoning_text.*` events.
...
Runtime selection is explicit. `openai-public` accepts only the frozen public request and event families. The narrow `codex-http-relay` acceptance profile uses the same public request semantics but permits three observed non-semantic server diagnostics, `codex.rate_limits`, `codex.response.metadata`, and `responsesapi.websocket_timing`; none enters normalized output or durable state. Other unknown events and every unknown output Item fail closed. No profile is inferred from provider id, hostname, URL, or model.
