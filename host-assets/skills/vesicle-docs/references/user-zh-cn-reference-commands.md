...
紧凑面板中有详情未显示完整时,阅读区会展开到应用页眉与页脚之间;已完整显示的短内容保持紧凑。阅读区只读,打字与粘贴不会修改原输入。返回保留选择、备注和光标;再次打开恢复阅读位置,缩放终端也按原文位置保持进度。新请求、新确认阶段或新的选择项重新开始阅读。迁移报告可查看全部检查项;会话预览仍是摘要。输入补全菜单、Workspace 页内对话框和 BTW 保留原有交互。
