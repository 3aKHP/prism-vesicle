...
| 直接打字 / 粘贴 | 在聚焦的确认选项上直接展开备注输入(Reject 本就常驻输入框;Esc 清空备注并回到选项) |
| Enter | 提交当前聚焦的选项;已输入的备注随决策一起提交 |
| Tab / Shift+Tab | 切到正文区阅读折叠的长摘要 / 命令详情 |
| Esc | 清空备注并把焦点移到 Reject |

| 按键(正文区) | 作用 |
|---|---|
| ↑ / ↓ | 逐行滚动折叠正文;位置指示(如 `… lines 5-8 of 30`)标明当前窗口 |
| Home / End | 跳到顶部 / 底部 |
| Tab / Enter / Esc | 返回选项区(Enter 在正文区**不会**提交决策) |

问题面板的问题文本同样按行预算折叠并可滚动;质量决策面板是固定选项列表,无正文区。面板提示行会随当前区显示可用键位。
