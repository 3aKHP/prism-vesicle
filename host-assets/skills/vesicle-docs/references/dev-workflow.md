<!-- Generated from docs/dev/WORKFLOW.md — do not edit. -->
...
6. The tag workflow rejects a lightweight tag, a tag/version mismatch, or a tag whose commit is outside the remote `main` history. If validation succeeds, it reruns every shared gate, uploads the versioned artifacts, creates the GitHub Release and checksums, then publishes npm through Trusted Publishing. The publication channel is derived from the validated version: `*-rc.*` release candidates publish to the npm `next` dist-tag as a GitHub prerelease that does not make `latest` (the beta line keeps tracking `latest`), stable releases publish to `latest` and make `latest`, and the historical beta line keeps its documented policy of tracking `latest`.
