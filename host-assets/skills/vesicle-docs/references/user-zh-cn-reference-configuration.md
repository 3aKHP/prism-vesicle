<!-- Generated from docs/user/zh-CN/reference/configuration.md — do not edit. -->
...
- `openai-public` 是官方 `api.openai.com` 的公开协议档案,支持 HTTP/typed SSE,也可显式选择 `responsesTransport: websocket`。它保留有序 Items、精确 `call_id`、无状态加密 reasoning、会话级 WebSocket continuation,以及在模型条目声明 `capabilities.remoteCompact: true` 后的 `/responses/compact`;当该独立端点返回 404 时(例如 Codex 订阅中转类后端),会自动退化为向 `/responses` 追加 `compaction_trigger` 哨兵项的内联压缩,无需额外配置。它也准许供应商内置联网搜索的声明与 `web_search_call` Item/事件(配合模型条目的 `builtinWebSearch` 能力与 `/websearch` 开关)。这是应用层协议声明,不代表 TLS/HTTP2 网络指纹与 Codex 相同。
