...
与配置目录无关的显示环境变量:`VESICLE_REDUCED_MOTION=1` 关闭启动画面和终端标题 working 标记的动画(冻结为静帧),适合对动态画面敏感或低性能终端。`VESICLE_THEME=dark|light|default|auto` 指定界面主题,四值语义:`dark`/`light` 强制对应主题;`default` 跟随终端自身的明暗模式(未报告时回退到深色);`auto` 按本地时间切换(07:00–19:00 浅色,其余深色)。`VESICLE_TERMINAL_TITLE=auto|on|off` 控制是否把宿主状态和会话标题投影到交互式终端标签页;`auto` 只在真实 TTY 中启用。`VESICLE_DISABLE_TERMINAL_TITLE=1` 是进程级强制不写开关,包括退出清理。已有 durable session title 会直接显示;没有标题时使用清洗后的项目 basename 回退。标签页状态槽使用 idle 的 `·`、working 的低频 `‹`/`◇`/`›`/`◇`、以及等待用户输入时的 `!`。无效值会给出一条诊断并回退到 `default`,不会被静默当成 `auto`。会话内可用 `/theme` 临时切换(优先级更高),启动时也可用 `--dark`/`--light` 进程级标志选择;项目级持久化见[下文](#项目主题偏好可选)。
