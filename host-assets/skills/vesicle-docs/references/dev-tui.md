...
- The same patch also carries the interim Markdown backslash-escape fix in `parser.worker.js` (upstream issue [anomalyco/opentui#1369](https://github.com/anomalyco/opentui/issues/1369), PR [#1370](https://github.com/anomalyco/opentui/pull/1370); fork branch `vesicle/fix-markdown-escape`): CommonMark `\X` escapes render as the escaped character — the worker conceals the backslash byte of `backslash_escape` nodes and drops the escape styling from the remainder. The component test `tests/component/tui/markdown-escape.test.tsx` asserts the rendered frame. This worker hunk is temporary: it is removed when Vesicle adopts the self-maintained OpenTUI fork baseline (Option B in the fork feasibility plan), which carries the same fix upstreamably.
