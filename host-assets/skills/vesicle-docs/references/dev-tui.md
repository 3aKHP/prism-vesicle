...
- The TUI runs on the self-maintained OpenTUI fork: `@3akhp/opentui-core@0.5.3-zv2` plus `@3akhp/opentui-solid@0.5.3-zv3` (upstream base v0.5.3 carrying the Vesicle patch queue: the worker-side backslash-escape fix, Markdown/TextTable selection colors, and the two native editor repairs for Issues #89/#99; provenance and native-artifact hashes live in the fork's GitHub Release `v0.5.3-zv2`). Markdown backslash escapes (`\~`, `\*`, …) therefore render as the escaped character natively in the fork's parser worker — the interim host-side transform and the 0.4.3 selection patch are gone. darwin-x64/arm64 resolve upstream `@opentui/core-darwin-*@0.5.3` natives through the fork's optional dependencies (byte-identical base: macOS gets the JavaScript fixes but not the two native editor repairs — disclosed in the fork release notes, not a platform narrowing). The parser worker resolves through the fork's `./parser.worker` export; native library selection stays the fork loader's job, with the host pinning the platform-package path before the first FFI touch (`src/tui/native-runtime.ts`). `vesicle debug markdown-runtime` reports the resolved native asset (descriptor key, or the runtime-pinned path in standalone binaries) and performs real native Markdown drag selection; package/binary/installer smoke runs it at each distribution boundary, its `escape` field asserts the fork worker's concealed tuples, and `tests/component/tui/markdown-escape.test.tsx` asserts the rendered frame.
- The published fork packages carry packaging defects bridged by tracked Bun patches, each with a removal condition tied to the next coordinated fork release. The solid patch rewrites `@opentui/*` specifiers the fork release left unrewritten in its runtime and declaration files (`jsx-runtime`, `components`, the bun-plugin/transform scripts, and the matching `d.ts` files). The core patch restores the Markdown selection propagation the retired 0.4.3 patch had and the fork port dropped (prose renderables, list markers, fenced code — the fork itself only propagates to tables), points the linux stub chunks' file-imports at the packaged `.so` files so re-bundling channels embed a real ELF, and exports the asset-descriptor helpers the host resolution and diagnostics use. The precompiled npm entry bundles the patched core JavaScript while leaving the parser worker and platform-native packages external, so installed consumers never depend on package-manager patch support or load a second unpatched core.
