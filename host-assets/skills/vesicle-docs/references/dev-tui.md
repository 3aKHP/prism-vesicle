...
- Picker panels and self-opened confirms (YOLO, migration review, rewrite confirm) own the bottom area while active on both pages. The four mid-turn decision prompts (permission, gate, question, quality) own the bottom on the Chat page; on the Workspace page they collapse to a one-line pending strip above the composer (`#268` item 3) — the workbench keeps its three-region keyboard and loses only the strip's one row (paid from the main-row budget, never from `bottomHeight`), the full panel answers on Chat, and the composer refuses Enter (no send, no queue; draft kept) until the decision resolves. Side panes may be hidden so the active controls remain legible.
- Decision prompts with folded body text (gate summary, permission detail, question text) carry a two-zone keymap (`#268` item 4): `Tab`/`Shift+Tab` toggle between the options zone and a body zone where `↑/↓` scroll, `Home`/`End` jump, and `Tab`/`Enter`/`Esc` return — Enter never submits from the body zone. Plain typing or paste on a focused confirm option auto-arms its note on gate and engine-switch prompts (the explicit "Tab note" toggle is gone); permission prompts have no confirm-note surface end to end, so typing on their confirm is swallowed; their notes live on Reject, whose composer renders whenever it is focused (it previously keyed on a feedback mode no code path ever set and was unreachable); the fold marker becomes an accent affordance row in the options zone (`▸ N lines folded · Tab to read`) and a bright position row in the body zone (`▾ lines 5-8 of 30`); the rail column beside the body is dim in the options zone and brand-colored in the body zone, and a new prompt resets zone and offset. The scroll extent is reported by the rendering component (`registerBodyExtent`), so the offset clamps to the real window.
...
- Chat-page `Ctrl+R` regenerates the latest completed turn as a new horizontal candidate; Workspace owns the same key for its existing file-reload action. `Ctrl+B` opens the candidate-tree panel from both pages: it sits after bottom-surface modals (an open panel keeps its keys) and above Workspace routing, and supersedes the composer's Emacs backward-char (bare `←` and `Meta+b` word movement remain). The `Ctrl+O` page switch outranks every bottom-surface modal (`#268` item 3) so the user can reach the other page while a prompt or picker is open; only the page beneath changes, the panel keeps every other key. Cross-platform letter shortcuts must remain representable by traditional VT input: do not require Shift to distinguish a Ctrl+letter binding (traditional terminals collapse `Ctrl+Shift+R` to the same `0x12` as `Ctrl+R`), and do not make an Alt/Option+letter binding the sole trigger because host overlays and terminal Meta configuration can intercept or reinterpret it. Shortcut regression tests parse real terminal sequences through OpenTUI before dispatch instead of constructing idealized modifier objects.
...
- The TUI runs on the self-maintained OpenTUI fork: `@3akhp/opentui-core@0.5.10-zv5` plus `@3akhp/opentui-solid@0.5.10-zv5` (upstream base v0.5.10; provenance and native-artifact hashes live in the fork's GitHub Release `v0.5.10-zv5`). The fork patch queue over the upstream base is JavaScript-side and ships in the core package for every platform: the worker-side markdown backslash-escape fix (`\~`, `\*`, … render as the escaped character in the fork's parser worker), Markdown/TextTable selection colors propagated to prose, list markers, and fenced code, the editor-view grapheme-boundary cursor snap, and an opt-in strict double-tilde strikethrough flavor (host wiring tracked in issue #294). The 0.5.3-era fork-native editor repairs (Vesicle #89/#99) are absorbed on this base — the wrap-reflow lineage by upstream's byte-accurate layout rewrite (#1428), the cursor-boundary lineage by the fork's JS snap plus upstream's grapheme/cursor repairs (#1447/#1449) — so darwin-x64/arm64 resolving upstream `@opentui/core-darwin-*@0.5.10` natives through the fork's optional dependencies is no longer a narrower platform set. Fork-built linux/win32 platform packages ship stripped native libraries with detached release symbols since 0.5.10 (#1437). The parser worker resolves through the fork's `./parser.worker` export, and native library selection is entirely the fork loader's job — dynamic platform-package import, `OTUI_ASSET_ROOT` relocation, and bunfs-embedded libraries in compiled binaries; the host adds no pin and no fork/upstream selection logic. `vesicle debug markdown-runtime` forces the native load through that loader (ok means the library really dlopen'ed); where the fork's installed-package asset table resolves (the source channel) it reports the native entry with `source: "asset-table"` — a table that resolves without a native entry fails the probe as a fork shape change — while npm-bundle installs and compiled binaries carry an inlined or embedded copy of the runtime whose asset table cannot resolve the installed layout and report `source: "forced-load"` with no path rather than inventing one. The diagnostic also performs real native Markdown drag selection; package/binary/installer smoke runs it at each distribution boundary, its `escape` field asserts the fork worker's concealed tuples, and `tests/component/tui/markdown-escape.test.tsx` asserts the rendered frame.
...
- The TUI projects host state to the terminal tab through the OpenTUI renderer's `setTerminalTitle()` boundary. The one-column status slot is `■` for idle, a same-width `◰`/`◳`/`◲`/`◱` quadrant-square pulse for working, and `▣` when a gate, permission, question, or other user decision is pending; working advances every 800ms and freezes at `◰` under `VESICLE_REDUCED_MOTION=1`. The working frames are the same-block quadrant squares U+25F0–F3 because they keep a pixel-stable advance across frames in the Windows Terminal tab strip, where the previous diamond pulse visibly shifted the following title text; the static markers `■`/`▣` (U+25A0/A3) share that square family so all three states read as one silhouette (#290). The human-observation frame probes live in `scripts/probe/terminal-title-jitter-probe.ts` (#263) and `scripts/probe/terminal-title-marker-probe.ts` (static-marker weight). A durable session title is displayed directly without a product prefix; untitled sessions fall back to `Prism Vesicle · <safe project basename>` and then the engine id. `VESICLE_TERMINAL_TITLE=auto|on|off` controls TTY admission, while `VESICLE_DISABLE_TERMINAL_TITLE=1` is a hard no-write override. Setup uses the fixed `Prism Vesicle Setup` title. Renderer shutdown, suspend/resume, and external-editor return clear or force-reproject through the same host-only controller; non-interactive output never contains title control sequences. If a Windows Terminal profile sets `suppressApplicationTitle`, the terminal may hide application-provided titles. Automated title-consumer evidence covers the Linux/WSL source TUI PTY; native Windows Terminal acceptance is covered by the 1.0.0-rc.2 closed group test on the installed build (working-marker anchor stability, #263).
