...
- Read, list, stat, and grep operations are limited to `assets/`, `source_materials/`, `workspace/`, `test_runs/`, `novels/`, `reports/`, the scratch root `tmp/`, and the read-only `skills/` mount described below.
- The `skills/` logical root mounts the session's activated Skills behind the ordinary read tools. It is resolver-backed like `assets/` (never a project directory), gated on the shared activation registry — activating a Skill mounts it, rewind or compaction loss unmounts it — and always resolves the frozen catalog winner, so a shadowed same-named copy (for example a losing harness copy) is never reachable through the mount. Enumeration (`list_directory`, `grep_files`) follows the frozen load-time inventory (`SKILL.md` plus indexed resources, bounded by the per-Skill resource limit); direct `read_file`, `stat_path`, and `view_image` resolve the live skill root through the same virtual-root hardening as `read_skill_resource`, so a file added after the catalog froze is readable when addressed directly but stays unlisted and unsearched until a new session.
- Mounted `read_file` accesses are UTF-8 text-only, cap at the shared 256 KiB Skill text limit with a truncation note, and refuse byte-slice (`offsetBytes`/`maxBytes`) reads; `view_image` reaches bundled images through the same attachment path as project images. `grep_files` over the mount reuses the shared excerpt and total-output caps and skips resources exceeding the 256 KiB text limit, disclosing the skipped paths in a result note. Every write, move, copy-target, and delete operation rejects `skills/` paths as a read-only mount.
- Reading through the mount records a plain `file_operation` event rather than a `skill_resource_read` event, so `read_skill_resource` remains the provenance-faithful reader and bundled Skill procedures keep steering to it; the mount exists so the ordinary read tools — especially `grep_files` — meet the model where it already reaches.
