...
- Read, list, stat, and grep operations are limited to `assets/`, `source_materials/`, `workspace/`, `test_runs/`, `novels/`, `reports/`, the scratch root `tmp/`, and the read-only `skills/` mount (full contract in "Read-only `skills/` mount" below).
...
- Read-tool schemas derive their root names from the canonical taxonomy. `list_directory({ path: "." })` queries a virtual logical root containing only the readable namespaces; it never lists the physical project root or exposes Host infrastructure. In that special discovery mode, `detail: "names"` returns logical root names; for every ordinary directory it returns file paths only.
- `stat_path` treats an allowed missing path as a successful `{ type: "not_found" }` observation. `list_directory` is the single provider-visible directory query: `detail: "full"` returns typed entries and metadata, while `detail: "names"` returns file paths only. Both modes also return `fileCount`, `directoryCount`, `otherCount`, and `empty`, so an empty result is distinguishable from a directory containing only subdirectories, symlinks, or special entries. They share the same structured status, entry and traversal bounds and support ordinary roots plus the layered `assets/` namespace and the activated-`skills/` mount; `list_directory({ path: "." })` lists `skills` as a read-only root whenever a session Skill mount is wired.
- `list_directory` is the single model-visible directory query surface. `list_files` is removed from the alpha Harness contract; use `detail: "names"` when only file paths are needed.
...
- Directory listing exposes symbolic-link entries without following them. Recursive operations and outputs remain bounded by 500 entries, 2,000 visited directories, and depth 8.
- File-capable Engine turns receive a bounded `<project_state>` orientation block as live Host prompt context, not session identity or conversation history. In-process pauses reuse the frozen snapshot; process restarts and new top-level turns observe again. Fresh and summary SubAgents with `list_directory` receive their own snapshot; fork SubAgents inherit the parent's frozen snapshot. It exposes only per-root file counts and absent/empty/inaccessible/truncated state, never contents or physical paths, and is fixed for that loop or delegated task so later mutations can make it stale. The model-writable roots are created explicitly at a new session's birth (best effort; failures surface as a host warning and an honest `inaccessible`/`absent` reading), so a fresh project's first observation reads `empty` rather than `absent`; the read-only `assets` root is excluded, and resumed sessions do not recreate roots the user deleted.
...
The `skills/` logical root mounts the session's activated Skills behind the ordinary read tools (`stat_path`, `list_directory`, `grep_files`, `read_file`, `view_image`). It is resolver-backed like `assets/` (never a project directory) and exists so the read tools — especially `grep_files` — meet the model where it already reaches when searching bundled documentation.

- **Lifecycle and resolution**: gated on the shared activation registry — activating a Skill mounts it, rewind or compaction loss unmounts it — and always resolves the frozen catalog winner, so a shadowed same-named copy (for example a losing harness copy) is never reachable through the mount.
- **Enumeration vs direct reads**: `list_directory` and `grep_files` follow the frozen load-time inventory (`SKILL.md` plus indexed resources, bounded by the per-Skill resource limit), while direct `read_file`, `stat_path`, and `view_image` resolve the live skill root through the same virtual-root hardening as `read_skill_resource`; a file added after the catalog froze is readable when addressed directly but stays unlisted and unsearched until a new session.
- **Read hardening**: mounted `read_file` is UTF-8 text-only, caps at the shared 256 KiB Skill text limit with a truncation note, and refuses byte-slice (`offsetBytes`/`maxBytes`) reads; `view_image` pre-flights the shared image attachment cap before buffering and reaches bundled images through the same attachment path as project images. `grep_files` reuses the shared excerpt and total-output caps and skips by live size — resources exceeding the 256 KiB text limit and files deleted from the skill root since the catalog froze — disclosing every skipped path in a result note. Listing entries that vanished or changed type since the freeze are omitted and the listing reports `truncated: true`. Read failures surface the skill-relative path and failure code only, never the absolute host root.
- **Write rejection and provenance**: every write, move, copy-target, and delete rejects `skills/` paths as a read-only mount. Reads through the mount record plain `file_operation` events (including `view_image`'s `operation: "view"` alongside its structured attachment) rather than `skill_resource_read` events, so `read_skill_resource` remains the provenance-faithful reader and bundled Skill procedures keep steering to it.
...
- Process tools (`shell_exec` and `run_skill_script`) are outside guarded file-tool authority. Host-process mutations taint checkpoint completeness and must never be described as rewind-safe.

## Permission Runtime

- Permission modes change approval friction and never widen the effective tool surface or bypass runtime guards.
- MANUAL asks for every model-visible execution. INERTIA auto-allows observation tools and asks for mutations, Skill scripts, and arbitrary commands. MOMENTUM auto-allows observation, mutation, and the structured `skill_exec` class but asks for `arbitrary_exec`; YOLO auto-allows all effective tools.
...
- The shared child-environment allowlist includes `APPDATA` and `XDG_CONFIG_HOME` when set, preserving standard user-config discovery for shell self-invocation on Windows and custom XDG installations. Provider secrets and Vesicle-specific overrides (`VESICLE_CONFIG_DIR`, `VESICLE_PROVIDERS_FILE`) are not inherited; Skill host injection retains precedence. Environment-policy version 2 is also bound into the shell approval hash: a pending version-1 request remains resumable and rejectable, but allowing it fails before execution. Reject the old request and issue a new call under the current policy; never rewrite old plans or replay commands automatically.
...
- Background processes return a short managed task id, persist bounded state under ignored `.vesicle/processes/`, and deliver completion as host-owned context: folded into the next provider round while a turn is active, or through an idle auto-continuation between turns (debounced and coalesced; a failed delivery pauses and retries after the next user input).
...
- Structured-argv Skill script execution (`run_skill_script`) uses the same Process Runtime timeout, output cap, cancellation, and cleanup as `shell_exec` but spawns no shell. It selects one fixed, catalog-pinned script from an activated Skill, rechecks the resource hash immediately before execution, and fails closed on drift. It is available independently of the `shellExec` configuration gate, uses the separate `skill_exec` permission class, and taints checkpoint completeness as a host process: MANUAL/INERTIA ask, while MOMENTUM/YOLO auto-allow. Script extension resolution maps `.sh`, `.py`, `.js`/`.mjs`/`.cjs`, `.ts`, and `.ps1` to their interpreters; `.ps1` prefers PowerShell 7 then Windows PowerShell 5.1 on Windows and `pwsh` only elsewhere, executing with `-NoLogo -NoProfile -NonInteractive -File` (never `-Command` or `-ExecutionPolicy Bypass`). The logical interpreter identity (`pwsh`, `powershell-5.1`, `sh`, etc.) appears in model-visible events, never the absolute executable path.
...
- Tool-surface visibility rules for the web family: when a session's provider-native built-in web search is enabled, the host `web_search` (Tavily) tool is removed from the surface so the model is never offered two competing search paths; without a resolvable `TAVILY_API_KEY`, the whole Tavily family (`web_search` / `web_fetch` / `web_map` / `web_crawl` / `web_research`) is hidden rather than offered as guaranteed-failing calls. The built-in search itself is not a tool call and never passes through the Tool Permission Runtime; it is governed by the `/websearch` toggle (see [`PROVIDERS.md`](./PROVIDERS.md)).
...
- Opt-in MCP output persistence: when `mcpOutputPersistence: true` is set in `.vesicle/preferences.yaml`, every MCP call's text result and decoded images are also written under `tmp/mcp-output/<sessionId>/` (and `.../blob/` for images) as native files with tool- and argument-derived names. The inline tool result is unchanged; persistence is an additive, best-effort durable copy the model re-reads with `read_file`/`grep_files`/`view_image`. Off by default; advertised via a system-prompt hint only for engines with MCP tools. Persisted outputs are scratch: not rewind-safe and never auto-cleaned.
- MCP output auto-truncate: the `mcpOutputAutoTruncate` sub-toggle (requires persistence) replaces an MCP text result at or above 32 KiB with a 4 KiB inline preview plus a reference to the persisted full copy, so a single large result cannot dominate the next request. Reads as off unless persistence is also on.
- `read_file` accepts optional `offsetBytes`/`maxBytes` for a bounded byte slice that never loads the whole file, and `grep_files` caps each match-line excerpt at 500 characters; both bound what a large persisted output (or a giant single line) can add to context.
- `grep_files` supports `outputMode` (`content` for match entries with text, `files_with_matches` for just file paths that contain matches, `count` for per-file hit counts) and optional `contextLines` (0–10 lines of surrounding context per match in content mode). A host-side 32 KiB output-text ceiling in content mode prevents a single search from dominating context; `maxMatches` limits match entries in content mode and file count in the other two modes.
