...
- Gemini `thought` and `thoughtSignature` metadata remain attached to the original provider-native parts and are replayed as those parts on the next request instead of being reconstructed from assistant prose. The adapter never emits a part whose only field is an empty text string: Gemini streaming can end a response with such a bare empty text part (the documented carrier shape for a trailing signature), and a signature-less one carries nothing the endpoint validates, so it is dropped at capture and at replay — while an empty text part that does carry a signature is replayed verbatim. A message that serializes to zero parts is omitted rather than sent as an empty-text placeholder.
