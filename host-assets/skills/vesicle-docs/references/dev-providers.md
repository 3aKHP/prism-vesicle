...
- Provider-native built-in web search is capability-gated by `capabilities.builtinWebSearch` with a session default from the model entry's `webSearchDefault` and a per-session `/websearch` override. It is model capability, not a host tool: permission modes do not govern it, and it reaches adapters only as the normalized `VesicleRequest.webSearch` declaration flag. Adapters without built-in search support leave the flag inert. Within `openai-responses`, the flag is consumed only by the search-admitting profiles (`openai-public`, `deepseek-subset-2026-08-19`), which declare the bare `web_search` tool, admit `web_search_call` Items and events, and persist the normalized report; see [`OPENAI_RESPONSES_CONFORMANCE.md`](./OPENAI_RESPONSES_CONFORMANCE.md). Gemini `generateContent` consumes the flag by appending a standalone `{googleSearch:{}}` tools entry (including alongside function declarations), then normalizes candidate `groundingMetadata` queries and optional URI/title citations; Gemini does not receive grounding metadata in replay because its protocol defines no replay item.
