...
- Gemini adapters map the system prompt to `systemInstruction`, conversation to `contents`, and tool results to `functionResponse` parts. A `user` Content holds either `functionResponse` parts only or ordinary multimodal (text/`inlineData`) parts only; tool-result images serialize as a separate following ordinary `user` Content, and later ordinary user messages are never merged into a function-response Content. Mixing the part kinds in one Content is rejected by the endpoint.
