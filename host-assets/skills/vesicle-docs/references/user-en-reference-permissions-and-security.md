...
- Deletion and relocation go through the same guards: `delete_file` removes files only, `delete_directory` removes empty directories only (never a fixed writable root), and `move_file` / `move_directory` never overwrite an existing target. All four are mutate-class tools, so INERTIA and MANUAL modes ask before they run.
