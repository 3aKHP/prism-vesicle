...
- Fixed-height TUI rows now opt out of implicit OpenTUI word wrapping, and shared truncation and padding measure terminal display columns instead of JavaScript string length. Rewind panels account for visible history and compact confirmation metadata so current-point, warning, and hint rows remain separate; Permission and YOLO panels reserve bounded command or warning rows so approval controls remain visible.
