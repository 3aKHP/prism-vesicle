...
- Added pinned Biome correctness linting for TypeScript and TSX source, scripts, and tests. Formatting and import assists remain disabled, Harness assets stay outside the lint scope, the reusable CI/release build enforces `bun run lint` before typechecking, and an opt-in tracked pre-push hook blocks pushes with lint diagnostics.
- Added the first-party `stage` consumer narrative Engine. `/stage <character-card-path> <scenario-card-path>` freezes the supplied Module A and Module B content unchanged, emits bounded compatibility warnings for unusual card structure, renders the released Stage context template, persists the host-created opening assistant message before the first player action, and freezes source hashes plus rendered context for resume. Stage exposes no model-visible tools, gates, MCP surface, or default rewrite; it validates the shared three-part packet and records `stage.prose` quality observations. Its player surface now keeps prose primary, compacts the HUD, hides complete logic comments by default, and toggles an individual opening or valid packet to its exact raw source on click or keyboard activation without changing provider history or session JSONL.
...
- Rewrote `STATUS.md` as an implementation-inventory index rather than a duplicate of the dev contracts: replaced the prose scope section and the two oversized known-limit paragraphs with a grouped capability-state table, a classified limits list, a refreshed repository tree, and links into the authoritative `docs/dev/` documents and user-manual advanced chapters. No runtime behavior, contract, or tool surface changed.
...
- Composer-owned arrow keys now consume their original OpenTUI event before
  ...
  also scroll the message stream or sidebar. Modified arrows are consumed as
  composer no-ops when they have no composer action, and `Ctrl+C` now consumes
  immediately while preserving its existing copy-selection / double-press exit
  contract.
...
- Refreshed the complete bundled V10 baseline to exact Neural Narratology Release `harness-20260720-3` / `prism-engine-v10@10.1.0-rc.1` from commit `90f65c952bd5c84da9d1c5a22fbe87c3c583a70a`. The package, standalone payload, and rollback baseline carry the exact 73-file manifest inventory, profile-declared host briefs, compact quality projections, raw Stage bootstrap template, and static prompt-asset ledger.
