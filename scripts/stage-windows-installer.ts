...
export const WINDOWS_COMMAND_ALIAS = "vesicle.cmd";
export const INSTALLER_PAYLOAD = [WINDOWS_EXECUTABLE, WINDOWS_COMMAND_ALIAS, "harness-manifest.json", "assets", "host-assets", "LICENSE"] as const;
...
export async function stageWindowsInstaller(rootDir = process.cwd()): Promise<string> {
  ...
  for (const path of INSTALLER_PAYLOAD) {
    const source = path === WINDOWS_COMMAND_ALIAS ? resolve(rootDir, "installer", path) : resolve(rootDir, path);
    await cp(source, join(stageDir, path), { recursive: true, force: true });
  }
