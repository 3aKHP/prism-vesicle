 ...
 * discovery winners (`project`, `user`, `harness`, `host`) plus enabled Skill
 * Store snapshots (`installed` scope), which discovery itself does not cover.
 * Collision precedence is `project` > `user` > `installed` > `harness` >
 * `host` — a shared project convention outranks personal authoring, which
 * outranks an explicit installation, which outranks the verified Harness
 * baseline, which outranks package-owned Host Skills; losers produce one
 * `shadowed` diagnostic each.
 *
...
import { resolveFilesystemSkills } from "./catalog-sources";
...
export async function resolveSkillCatalog(
  ...
  const inspection = await resolveFilesystemSkills(rootDir, env, options);
  ...
  for (const installed of await loadInstalledSkills(env, diagnostics)) {
    ...
    if (existing && existing.scope !== "harness" && existing.scope !== "host") {
  ...
  for (const [name, skill] of winners) {
    if ((skill.scope === "user" || skill.scope === "host") && userDisabled.has(name)) {
      winners.delete(name);
      diagnostics.push({ kind: "shadowed", message: `Skill "${name}" in scope "${skill.scope}" is disabled and excluded from the catalog.` });
