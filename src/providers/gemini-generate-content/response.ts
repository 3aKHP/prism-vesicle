...
import { type GeminiGroundingMetadata, type GeminiPart, type GeminiResponse, isBareEmptyGeminiTextPart } from "./types";
...
export function responseFromGeminiParts(args: {
  ...
  const thinkingBlocks: ProviderThinkingBlock[] = needsReplay
    ? normalizedParts
        .filter((part) => !isBareEmptyGeminiTextPart(part))
