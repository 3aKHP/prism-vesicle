  ...
  return (
    <Show when={props.source} fallback={
      <box flexDirection="column" width="100%">
        {props.parsed.hud
          ? <box flexDirection="column" width="100%">
              <ThemedText content={`◇ ${props.parsed.hud.summary}`} fg={palette.textDim} />
              <box height={1} />
            </box>
          : <box height={0} />}
        {normalStageRenderParts(props.parsed).map((part) => part.kind === "anchor"
          ? <box id={part.id} height={0} />
          : <ThemedText id={part.segment.id} content={renderMarkdownPlainText(part.segment.raw)} fg={palette.textPrimary} />)}
      </box>
    }>
      <box flexDirection="column" width="100%">
        {props.parsed.segments.map((segment) => <ThemedText id={segment.id} content={segment.raw} fg={palette.textPrimary} />)}
      </box>
    </Show>
  );
}
