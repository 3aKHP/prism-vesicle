...
| TUI | Decision-prompt body zones: gate/permission/question prompts wrap their folded body text behind a Tab-switched reading zone (`↑/↓` scroll, `Home`/`End` jump, Enter never submits) with a scroll position indicator, and notes arm by typing or paste on the focused confirm option (the old "Tab note" toggle is gone) | released |
