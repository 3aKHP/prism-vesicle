...
| TUI | Shared bottom-panel reading: Tab/Shift+Tab reads every decision, confirmation and selected picker item; truncated content expands between header and footer, short content stays compact. Arrow keys/Ctrl+P,N and Home/End scroll; Tab/Enter/Esc only return, preserving notes, selection, cursor and source position across resize. Ctrl+O keeps Workspace decision strips and page-following user dialogs. Gate confirm notes still arm by typing/paste; permission notes remain Reject-only | implemented, unreleased |
