...
|-----------|-----------|-------|
| Assets | Bundled V10 Harness (`prism-engine-v10@10.3.3`, verified 73-file inventory) | released |
...
| Providers | Explicit Responses profiles: official OpenAI HTTP/SSE/WebSocket + remote compact (standalone endpoint with inline `compaction_trigger` fallback), and frozen MiMo/DeepSeek HTTP subsets | released |
...
| TUI | Shared bottom-panel reading: Tab/Shift+Tab reads every decision, confirmation and selected picker item; truncated content expands between header and footer, short content stays compact. Expanded titles appear once and wrap; gates and questions render Markdown, while permission commands/JSON remain literal. Arrow keys/Ctrl+P,N and Home/End scroll rendered rows; Tab/Enter/Esc only return, preserving notes, selection, cursor and text position across resize. Ctrl+O keeps Workspace decision strips and page-following user dialogs. Gate confirm notes still arm by typing/paste; permission notes remain Reject-only | implemented, unreleased |
| TUI | Two-page shell (Chat / Workspace, `Ctrl+O` and `/workspace`), page-aware header, sidebar renamed to Host; page switching (`Ctrl+O`) outranks every bottom-surface modal, and on the Workspace page mid-turn decision prompts (permission/gate/question/quality) collapse to a one-line pending strip above the composer — the workbench keeps its rows and keyboard, the full panel answers on Chat, and Enter neither sends nor queues until the decision resolves | released |
...
| TUI | Any-depth candidate tree: `/branch` / `Ctrl+B` browses every fork at every depth (including inactive subtrees) and switches to any candidate after a read-only file-preview confirm; full-manifest file bundles make the disk strictly equal to the selected candidate | released |
| TUI | Unified turn-focus cursor: `Alt+↑/↓` navigates every transcript turn-by-turn with wrap-around and highlighting; `Alt+←/→` reports `Ctrl+B`/`Ctrl+R` guidance when no switcher is armed | released |
...
| Skills | Agent Skills runtime (Phase 0–3): four-scope discovery with deterministic precedence, immutable versioned Skill Store, repository install/update/rollback/uninstall, model-visible activation/resources/scripts, `/skill`, and bundled first-party Skills (`vesicle-docs`, `skillify`, `novel-outline-v3`, `update-config`) | released |
| Web | Provider-native built-in web search: normalized `WebSearchReport`, `capabilities.builtinWebSearch` + `webSearchDefault` config, `/websearch` session toggle; consumed by OpenAI Responses (`openai-public`, `deepseek-subset-2026-08-19`) and Gemini `generateContent` | released |
| CLI | `vesicle config` management surface (`show`/`set`/`unset`, provider/model/MCP add & remove, non-secret `env-*`, `validate`) with comment-preserving atomic registry edits | released |
...
| `list_directory` | Read-only unified directory query; `.` is a safe virtual root, `full`/`names` results are structured and bounded, and the layered `assets/` namespace plus the activated-`skills/` read-only mount are supported |
| `grep_files` | Read-only; searches the readable project roots, the `assets/` namespace, and the activated-`skills/` mount |
| `read_file` | Read-only, with optional line ranges and `offsetBytes`/`maxBytes` bounded byte-slice reads (byte slices are project files only; the `skills/` mount is line-range-only with the shared 256 KiB Skill text cap) |
| `view_image` | Read-only, guarded image attachment (vision-capable models only), including images bundled under the `skills/` mount |
...
- Web tools are limited to the five Tavily host tools on ETL and Evaluate profiles. Provider-native built-in web search uses the normalized `WebSearchReport`, `capabilities.builtinWebSearch` + `webSearchDefault` config, the `/websearch` session toggle, and Tavily surface rules. OpenAI Responses consumes it on `openai-public` and dated `deepseek-subset-2026-08-19` (bare `web_search` declaration, admitted `web_search_call` Items/events, normalized queries/citations/call records, and call-item replay; frozen profiles stay fail-closed). Gemini `generateContent` consumes it with a standalone `{googleSearch:{}}` declaration that coexists with function declarations, normalizes `groundingMetadata` queries and optional citations, and intentionally does not replay grounding metadata.
...
- The Output Quality Guard ships deterministic findings, the six published document metrics, and durable per-target rewrite decisions on `develop`. The Semantic Judge is an **experimental** user-level override that defaults to `off` in `quality.yaml` and makes no production-quality or AI-authorship claim. In `observe` mode its findings are advisory only and never enter blocking policy; in the explicitly confirmed `rewrite` mode (a red two-stage confirmation in `/quality`, mirroring `/permissions YOLO`) Judge findings are promoted to blocking and enter the existing bounded Runtime rewrite lifecycle and post-image recheck. Both modes remain independent of the calibrated `semantic-rewrite@1` policy below.
...
- With no project lock, Vesicle automatically verifies and activates the bundled `prism-engine-v10`; rollback returns to that same baseline. A session whose recorded Harness identity differs from the active baseline (including identity-less pre-V10 sessions) resumes through a one-time explicit migration: an offline preflight report (blocking findings refuse the migration), a two-stage confirmation, a pre-migration archive under `.vesicle/sessions/archive/`, and a durable `session-migration` record that rebinds the effective identity and re-freezes the Skill catalog while history records keep their original identity; see [`docs/dev/SESSIONS.md`](./docs/dev/SESSIONS.md). Skill content that drifts without a Harness identity change has an explicit recovery path: `/skill refresh` re-freezes the session catalog at the current installation content (changed/removed/added report, idempotent no-op without drift, stale-hash activations retire until re-activated), and a resumed session with unchanged identity surfaces an advisory drift hint pointing at the command (#308). Pending-quality retries stay fail-closed across the drift, and SubAgent child sessions are not migrated.
