...
| `read_file` | Read-only, with optional line ranges and `offsetBytes`/`maxBytes` bounded byte-slice reads |
...
- Cross-process Skill Store index locking (`src/skills/store.ts`) and the enable/disable index write (`src/skills/disabled.ts`) rely on SQLite `BEGIN IMMEDIATE` file locking. Bun's bundled SQLite on Windows does not enforce cross-process file locks: a second process acquires `BEGIN IMMEDIATE` while the holder's transaction is open (verified on bun 1.3.14; `journal_mode=WAL` and `locking_mode=EXCLUSIVE` are equally affected, while two connections inside one process do exclude each other). Concurrent Store writes on Windows can therefore interleave their index read-modify-write; the atomic temp+rename write still prevents a torn index file, and the worst case is a dropped index entry recoverable by reinstalling the skill. Linux is unaffected. Tracked upstream at [oven-sh/bun#40659](https://github.com/oven-sh/bun/issues/40659).
