...
| Stage Engine | Consumer RP bootstrap with frozen Module A/B context, empty tool surface, runtime-packet observe, prose-first packet rendering | Implemented on development branch |
...
| Output Quality Guard | Target-aware deterministic findings + document metrics + opt-in experimental Semantic Judge + Semantic Rewrite Policy contract loader + durable decisions + developer benchmark runner | Implemented on development branch |
...
- Pushing a protected annotated `v<package.json version>` tag on the accepted `main` commit is the publication authorization. The workflow rejects tags outside `main`, reruns the shared release gates, creates the GitHub Release with SHA-256 checksums, and runs npm Trusted Publishing with provenance without normal browser dispatch or GitHub Environment approval. Windows signing is deferred and is not part of this publication authorization.
- Public [`CODE_SIGNING_POLICY.md`](./CODE_SIGNING_POLICY.md) and [`PRIVACY.md`](./PRIVACY.md) documents define the intended future Windows signing scope, mandatory per-request human approval, historical unsigned-artifact boundary, local data retention, external-service transfers, and user deletion controls. Windows signing is deferred until the project has a stronger basis for a signing provider, with no version deadline. `1.0.0-alpha.2` remains an explicitly disclosed unsigned release.
...
- Load multiple OpenAI-compatible provider/model profiles from the user-level provider config (`%APPDATA%\prism-vesicle\providers.yaml` on Windows, `$XDG_CONFIG_HOME/prism-vesicle/providers.yaml` or `~/.config/prism-vesicle/providers.yaml` elsewhere); the TUI can switch provider/model through a two-step `/model` picker or direct command forms. Direct input keeps its provider-first completion after the first argument space and provider-scoped model completion after the second, while also completing the established `/model <model>` active-provider shorthand. Provider files name `apiKeyEnv` variables only; actual secrets stay in the same user-level directory's `.env` file, with process environment variables used only as fallback.
- Slash-command definitions own their argument grammar, option sources, and canonical draft builders. The shared filtered Up/Down, Ctrl+P/Ctrl+N, Tab, Enter, and Escape interaction covers fixed values, configured provider/model values, refreshed artifacts and sessions, guarded Stage card paths, and optional command tokens such as `/engine <id> --summary`.
...
├── assets/
│   └── ...               # Exact 73-file V10 Harness manifest inventory
├── host-assets/
...
- Bundled and managed Harness verification, immutable managed installation, project selection, exact session identity, and whole-baseline rollback are implemented. With no project lock, Vesicle automatically verifies and activates bundled `prism-engine-v10@10.1.0-rc.1`; rollback returns to that same baseline. The Stage profile is a first-party consumer entrypoint: `/stage <character-card-path> <scenario-card-path>` freezes the supplied Module A/B content unchanged, emits bounded compatibility warnings for harmless card variation, renders and persists frozen bootstrap context plus the opening assistant message, and resumes that saved context even when source cards later drift. Stage has no model-visible tool, MCP, Agent, shell, or gate surface; it uses `runtime-packet` validation and `stage.prose` observe binding. The consumer view keeps prose primary, renders a compact HUD indicator, and hides complete bootstrap logic and valid packet Neural Chain comments until the player clicks that Stage message (or activates its focus with `Ctrl+Alt+S`) to inspect its exact raw source. This presentation state is not persisted; raw packets remain intact in provider history and session JSONL. Sessions recorded before the V10 migration have no Harness identity and fail closed on resume. `prism-agent/delegation@1`, `quality-guard/anti-ai-flavor@1`, the finite `quality-detector/document-metrics@1` registry, and `quality-judge/anti-ai-flavor@1` are enforced. The Guard validates the released Rule Pack, matches normalized protected prose with UTF-16 evidence offsets, and evaluates the six published document metrics without executing unknown signal kinds. Metric regex contracts fail closed on unsafe structure or excess total patterns, and every target shares a bounded match-work budget; exhausting that budget delivers an append-only `detector-budget-exhausted` inconclusive warning instead of blocking the turn or reporting clean. Those metrics remain experimental advisory findings even under Runtime's `rewrite` binding; they do not enter blocking policy or spend rewrite attempts. The Semantic Judge is a user-level experimental override, defaulting to off in `quality.yaml`; when explicitly enabled it uses a separately registered provider/model with no tools or normal conversation history, low generation variance, bounded output, strict JSON/rule/evidence validation, and at most one format repair. `observe` records findings, while confirmed experimental `rewrite` enters the existing two-attempt original-Engine lifecycle and post-image recheck for eligible Runtime or Stage prose. It remains independent of calibrated Policy activation and makes no production-quality or AI-authorship claim. Timeout, provider failure, invalid output, oversize targets, and profile drift deliver with durable inconclusive warnings; session metadata retains only hashes, bounded evidence, a secret-free profile identity, timing, request count, and bounded usage rather than the candidate or raw Judge response. The developer-only `vesicle quality benchmark` command separately runs an explicit provider/model matrix from a frozen plan, including a bounded per-evaluation Judge timeout that does not alter the interactive 15-second deadline; it rejects unsupported targets before any provider call, appends resumable hash-only measurement rows, reserves two-request format-repair budget before each evaluation, writes Wilson/slice reports without raw candidates or responses, and refuses live provider calls without `--allow-live`. It does not authorize semantic blocking. Stable literal and regex findings retain the target-aware lifecycle: successful prose mutations bind to independent project-relative artifact targets, every complete guarded post-image is reread before delivery, failing Runtime targets receive at most two original-Engine rewrites, and repeated per-target post-image hashes stop the loop. Exhaustion persists an append-only quality warning and decision point instead of reporting ordinary completion; the TUI can authorize one more original-Engine revision, use the current version with its warning, or stop without a provider call. Cancellation, provider failure, and process restart preserve the same decision, while Harness, Rule Pack, or experimental profile identity drift disables retry without preventing a local accept/stop record. Session rows mark interrupted or pending quality work, artifact rows mark paths with unresolved warnings, and a later clean post-image writes an explicit resolution. Observe bindings still cover Dyad, Weaver, Weaver-Orch, Scene Writer, and Stage; Evaluate and Chapter Reviewer reports remain excluded from recursive enforcement. Archive extraction, online Release discovery, channels, downloads, and automatic updates remain deferred; the offline CLI accepts an already-extracted pack directory.
