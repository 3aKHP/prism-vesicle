...
| TUI | Horizontal candidate branching: `/regenerate` re-runs the last turn as a switchable candidate (`Option+←/→`); append-only sibling subtrees off the shared user record, selection persisted | released |
...
- `/regenerate` re-runs the last turn as a new candidate by appending a sibling subtree off the shared user record (Model B: one user record, N candidate subtrees); old candidates are never deleted or garbage-collected, so the session JSONL grows with each regenerate and candidate switch, and `loadSessionSnapshot` (which reads the whole file) slows accordingly. Regenerate does not roll back the previous candidate's on-disk artifacts: a regenerated file-writing turn executes against the previous candidate's files, and switching candidates switches conversation state but not file state (Stage, which writes no files, is unaffected). Per-candidate file coexistence is a committed follow-up. Regenerate is refused while a turn is running, a confirmation/permission/question is unresolved, or a background SubAgent is still in flight.
